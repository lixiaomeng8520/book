...
还记得之前添加的配置吗？在这里我们使用它 - 容器知道如何获取配置，所以我们可以在这里轻松访问配置。有了这些配置，我们创建`PDO`类（这里如果失败可能会返回`PDOException`，你需要进行处理）来连接数据库。这里我调用了两次`setAttribute()`，这不是必要的，但是我发现这两个设置很有用，所以在例子里留了下来，你可以使用它们！最后，我们返回了connection对象。

另外，我们可以通过`$this->`来访问依赖，这里可以使用`$this->db`：
...
“路由”是我们将描述和附加功能的URL模式。Slim不使用任何自动映射或URL公式，所以你可以创建任何你喜欢的路由模式，映射到任何你喜欢的方法，非常灵活。路由可以链接到一个特定的HTTP动作（比如GET或POST），或者其他的。
...
在这个例子中，所有的路由都定义在index.php里，但是在实际使用中，这会使这个文件很冗长！重构你的应用，把路由放在不同的文件里是不错的方式，或者仅仅注册路由，而他们的回调实际上在别的地方声明。

所有的路由回调接收三个参数（第三个是可选的）：

* Request：包含了所有关于进站请求，头部，变量等信息。
* Response：我们可以向它添加输出和头部，一旦完成，它就会转化成客户端接收的HTTP响应。
* Arguments：URL里的命名占位符，它是可选的，如果没有的话，经常会被忽略。

Request和Response的强调说明了Slim 3是基于HTTP Messaging的PSR-7标准。使用Request和Response会让应用更容易测试，因为我们不需要产生实际的请求和响应，我们仅仅根据需要来操作对象就行。
